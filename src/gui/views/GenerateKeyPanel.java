...
public class GenerateKeyPanel extends JPanel {
	...
	private JTextArea taSymKey = GUIFactory.getInstance().createJTextArea(21, 120),
			taAsymPubKey = GUIFactory.getInstance().createJTextArea(8, 120),
			taAsymPrvKey = GUIFactory.getInstance().createJTextArea(13, 120);

	public GenerateKeyPanel() {
		JTabbedPane tpane = new JTabbedPane();
		JPanel panelSym = new JPanel(new BorderLayout()), panelAsym = new JPanel(new BorderLayout()),
				panelSymKey = new JPanel(new GridBagLayout()), panelAsymKey = new JPanel(new GridBagLayout());
		JScrollPane jscpSym = new JScrollPane(panelSymKey), jscpAsym = new JScrollPane(panelAsymKey);
		JLabel labelSymKey = new JLabel("Sym. key:"), labelAsymPubKey = new JLabel("Asym. pub. key:"),
				labelAsymPrvKey = new JLabel("Asym. prv. key:"),
				labelInfo = new JLabel("In Java exists no standard algorithem "
		...
		GridBagConstraints gbcSymLab = GUIFactory.getInstance().getConstrains(0, 0, 1, 1, 0, 0),
				gbcSymKey = GUIFactory.getInstance().getConstrains(1, 0, 1, 1, 1, 1),
				gbcAsymPubLab = GUIFactory.getInstance().getConstrains(0, 0, 1, 1, 0, 0),
				gbcAsymPrvLab = GUIFactory.getInstance().getConstrains(0, 1, 1, 1, 0, 0),
				gbcAsymPubKey = GUIFactory.getInstance().getConstrains(1, 0, 1, 1, 1, 0),
				gbcAsymPrvKey = GUIFactory.getInstance().getConstrains(1, 1, 1, 1, 1, 1);
		...
		JButton bttSym = new JButton("Generate sym. key"), bttAsym = new JButton("Generate asym. keys");
			...
			case "SYM":
				try {
					...
					List<String> list = WordProcessingFactory.getInstance().createWordWrap(
							Cypher.getInstance().generateSymKey(
									((CipherType) GenerateKeyPanel.this.cbSym.getSelectedItem()).getAlgor(),
									((CipherType) GenerateKeyPanel.this.cbSym.getSelectedItem()).getBits()),
							120, new LinkedList<String>());
					for (String s : list)
						GenerateKeyPanel.this.taSymKey.append(s + System.lineSeparator());
			...
			case "ASYM":
				...
				try {
					...
					tmp = Cypher.getInstance().generateAsymKey(
							((CipherType) GenerateKeyPanel.this.cbAsym.getSelectedItem()).getAlgor(),
							((CipherType) GenerateKeyPanel.this.cbAsym.getSelectedItem()).getBits());
					strKeyPub = WordProcessingFactory.getInstance().createWordWrap(tmp[0], 120,
							new LinkedList<String>());
					strKeyPrv = WordProcessingFactory.getInstance().createWordWrap(tmp[1], 120,
							new LinkedList<String>());
					for (String s : strKeyPub)
						GenerateKeyPanel.this.taAsymPubKey.append(s + System.lineSeparator());
					for (String s : strKeyPrv)
						GenerateKeyPanel.this.taAsymPrvKey.append(s + System.lineSeparator());
				} catch (NoSuchAlgorithmException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}
